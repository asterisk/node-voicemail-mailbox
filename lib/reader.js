...
function fsm(mailbox, channel, dependencies) {
  var fsmInstance = new machina.Fsm({
    ...
    playMenu: function(invoker, full) {
          ...
          if (full) {
              sounds = sounds.concat(invoker.messages.isNotEmpty() ?
                                     readerPrompts.introMessages :
                                     readerPrompts.introNoMessages);
          }

          sounds = sounds.concat(invoker.getMenu());

          var replacements = {
            messageCount: invoker.messages.getCount(),
            plural: invoker.messages.manyExist() ? 's': '',
            folder: invoker.messages.getCurrentFolder().recording
          };

          if (invoker.currentPrompt) {
            invoker.currentPrompt.stop();
          }

          invoker.currentPrompt = dependencies.prompt.create(
              sounds, channel, replacements);

          invoker.currentPrompt.play()
            .then(function() {
              invoker.transition('ready');
            })
            .catch(function(err) {
